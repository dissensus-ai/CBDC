...
Escalations (tier-conditioned amounts, sensitivity check) & 592 (1.6\% of settled) \\
\addlinespace
Latency mean                 & 537\,ms \\
Latency p50                  & 511\,ms \\
Latency p90                  & 768\,ms \\
...
The 36.8\% rejection rate reflects the simulation's tier-0 per-transaction limit (\$50) interacting with the lognormal transaction amount distribution, not a system failure mode: 14{,}631 of the 14{,}713 rejections are per-transaction-limit breaches and the daily cap never binds. Tier-0 wallets (55\% of the population) have the lowest per-transaction limits, and the heavy-tailed amount distribution frequently exceeds them. In a deployed system, tier distribution would reflect actual KYC levels, and limits would be calibrated to real-world spending patterns, yielding substantially lower rejection rates for compliant users. As a sensitivity check, running the same configuration with amounts drawn per-wallet from a tier-scaled lognormal (mean $= f \times$ that wallet's tier limit, via \texttt{pet\_aml\_sim.py --tier-amounts}) rather than one global distribution shared across all tiers drops the rejection rate to 8.4\% (3{,}375 of 40{,}000) at $f = 0.4$, with latency and throughput unchanged. This is not a single calibrated number but a closed-form function of $f$ and the shared amount-distribution shape parameter $\sigma$: with amounts lognormal and mean $f \times \text{cap}$, the per-tier rejection probability is $1 - \Phi\left((-\ln f + \sigma^2/2)/\sigma\right)$, independent of the tier mix. At $\sigma = 1$ this gives 1.8\%, 4.4\%, 7.8\%, and 11.6\% for $f = 0.2, 0.3, 0.4, 0.5$ respectively---so $f = 0.4$ is one plausible calibration among many, not a chosen answer, and the same closed form applied to the baseline's untiered distribution predicts a 36.4\% rejection rate against the 36.8\% observed. Both the 36.8\% baseline and the 8.4\% sensitivity figure are therefore explained by the ratio of typical spend to tier cap rather than by anything specific to the architecture.

The 82 sanctions-blocked transactions confirm that PSI screening correctly intercepts watchlist-matched wallets without revealing match status to the ledger operator. An earlier version of this simulation reported zero escalations, an artifact of processing order rather than of the architecture: risk tiers were assigned during batch risk propagation \emph{after} the full transaction loop completed, so tier-based escalation could never fire within the simulated flow. The simulator now runs risk propagation once per simulated day, immediately after that day's transactions settle, so each subsequent day's escalation check sees risk tiers updated from prior activity (the first day, with no prior history, necessarily sees only defaults). Under this corrected ordering the same two-day, 40{,}000-transaction run (seed 7) yields 27 escalations, all triggered by the risk-tier threshold rather than the transaction-amount threshold---consistent with an amount distribution (lognormal, mean \$120) whose tail rarely reaches the \$10{,}000 escalation amount at this sample size. This is a real, if small, validation of the tier-based escalation pathway rather than a null result by construction; it is not evidence about escalation rates in a deployed system, which would depend on the true prevalence and behavior of high-risk wallets. The tier-conditioned-amounts sensitivity check above illustrates exactly this sensitivity: it moves escalations from 27 to 592 (0.11\% to 1.6\% of settled transactions), a 22-fold change driven entirely by the amount-distribution calibration rather than by anything architectural, underscoring that the escalation count is a property of the input distribution and not a claim about real-world escalation rates.
